...
Interaction with the graph is mediated through the Model Context Protocol (MCP)~\cite{anthropic2024mcp}. This integration exposes over fifty specialized tools across eight categories, allowing agents to mutate, query, and analyze the graph structure. Table~2 summarizes the primary categories:
...
To prevent ``hallucinated permissions,'' the system implements a \emph{Restricted Context Protocol} using Role-Based Access Control (RBAC). We wrap the MCP client in a \texttt{RestrictedMCP} class that filters available tools based on the agent's specific role. For instance, ``Worker'' agents are physically barred from calling \texttt{source\_read}, forcing a unidirectional data flow: \texttt{Reader} $\rightarrow$ \texttt{Broadcast} $\rightarrow$ \texttt{Workers} $\rightarrow$ \texttt{Synthesizer}.
...
We govern the exploration-exploitation trade-off via a \emph{Graph Thermostat}. This meta-cognitive mechanism calculates the entropy of the graph by measuring the relative density of \texttt{Tension} nodes (conflict) and \texttt{Orphan} nodes (fragmentation). It dynamically toggles the swarm's strategy between \texttt{DIVERGE} (injecting chaos when the graph is stagnant) and \texttt{CONVERGE} (consolidating insights when the graph is fragmented).
